...
/**
 * Format a date for the PDF in UTC.
 *
 * Previously this used the server's local timezone via the default
 * toLocaleDateString behavior. PDFs generated on a US-hosted edge
 * (UTC) vs a developer's local machine (e.g. PST) could differ by
 * one day at midnight boundaries — same estimate, regenerated by
 * different infrastructure, showed different "Generated on" dates
 * for what was conceptually the same moment in time. Pinning UTC
 * makes generation deterministic across hosts and matches the way
 * timestamps are stored (ISO 8601 UTC in the database).
 */
...
export async function buildEstimatePdfBuffer({ estimate, branding = {} }) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // --- Totals --------------------------------------------------------
      // pdfkit advances `doc.y` after every `text(...)` call. We snapshot
      // the row Y before drawing both columns so labels and amounts stay
      // paired on the same row.
      //
      // After both text() calls, doc.y is set to wherever the LAST call
      // ended — which is the right-hand `value` column's final cursor.
      // If a value wraps (e.g. "$12,345,678.90" being narrower than its
      // column, this almost never happens — but a customer with a very
      // wide custom currency string can trigger it), the label's wrap
      // might land lower than the value's. We explicitly take the max
      // of the two final cursor positions before returning so the next
      // row always starts below BOTH columns, never colliding with the
      // tail of either.
      ...
      const renderTotalsRow = (label, value, { bold = false, size = 10 } = {}) => {
        ...
        const valueEnd = doc.y;
        doc.y = Math.max(labelEnd, valueEnd);
      };
