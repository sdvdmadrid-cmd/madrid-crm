...
export default function JobsPageClient({ initialList = null }) {
  const { t } = useTranslation();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { capabilities } = useCurrentUserAccess();
  const [jobs, setJobs] = useState(initialList?.data ?? []);
  ...
  const fetchJobs = useCallback(async ({ page = 1, append = false, search = "" } = {}) => {
    ...
    try {
      ...
      setJobs((prev) => (append ? [...prev, ...batch] : batch));
    } catch (err) {
    ...
    <main className={`${ws.page} ${jobStyles.jobsPage}`}>
      ...
      <section>
        ...
        <div className={jobStyles.filterBar}>
          ...
          {listTotal > jobs.length && !listSearch.trim() && !filterClientId ? (
            <button
              type="button"
              className={ws.btnSecondary}
            ...
            >
              {loadingMore
                ...
                : t("jobs.loadMore", { defaultValue: "Load more" })}
            </button>
          ) : null}
        </div>
        {filterClientId ? (
