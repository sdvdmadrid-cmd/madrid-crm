...
export default function EstimateClientPage() {
  ...
  const canRespond = status === "sent" || status === "changes_requested";
  ...
  const services = estimate?.services || [];
  const company = estimate?.companyProfile || {};
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-6 flex items-start justify-between print:mb-4">
          <div>
            ...
            {company.companyName ? (
              <div className="mt-1 text-sm font-medium text-slate-600">{company.companyName}</div>
            ) : null}
          </div>
          <div className="flex flex-col items-end gap-2">
            ...
            {visitWebsiteUrl ? (
              <a
                ...
                className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-400 hover:text-slate-900"
              >
                Visit Website
              </a>
            ) : null}
        ...
        {services.length > 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm mb-4 print:shadow-none">
            ...
            <div className="space-y-2">
              <div className="hidden grid-cols-[1fr_60px_90px_80px] gap-2 border-b border-slate-100 pb-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400 sm:grid">
                <span>Item</span>
              ...
              {services.map((s, i) => {
                ...
                return (
                  <div key={s.id || i} className="rounded-xl border border-slate-100 p-3 text-sm text-slate-800 sm:grid sm:grid-cols-[1fr_60px_90px_80px] sm:gap-2 sm:border-0 sm:p-0">
                    <span className="font-medium">{s.name || "-"}</span>
                    <span className="mt-2 flex justify-between text-slate-600 sm:mt-0 sm:block sm:text-right">
                      <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 sm:hidden">Qty</span>
                      {qty}
                    </span>
                    <span className="mt-1 flex justify-between text-slate-600 sm:mt-0 sm:block sm:text-right">
                      <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 sm:hidden">Unit</span>
                      {formatMoney(unit)}
                    </span>
                    <span className="mt-1 flex justify-between font-semibold sm:mt-0 sm:block sm:text-right">
                      <span className="text-xs font-semibold uppercase tracking-wide text-slate-400 sm:hidden">Total</span>
                      {formatMoney(total)}
                    </span>
                  </div>
                );
